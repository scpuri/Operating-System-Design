...
/* Used across multiple files; the position of the next character to write*/
extern int screen_x;
extern int screen_y;

extern char err_proc[128];

#define ROUND_OFF 0xFFFFE000
#define SIZEOF_LONG 4

/* Macro added by one of the fourDudes
 * Return a pointer to current processes PCB
 * */
static inline pcb_t * get_pcb()
{
	pcb_t * val;
	asm volatile("movl $0xFFFFE000, %%eax\n   \
			andl %%esp, %%eax"
			: "=a"(val)
			: 
			: "memory" );
...
/* Pop 32 bits off the stack into value */
#define popl()                 \
do {                                \
...
/* Pop 16 bits off the stack into value */
#define popw()                 \
do {                                \
 ...
 /* Make a call to iret to return to the saved EIP
  * and restore CS and EFLAGS  */
#define iret()	                        \
do {                                    \
	asm volatile("iret"                 \
			); 							\
 ...
 /* Preserve all general registers  */
